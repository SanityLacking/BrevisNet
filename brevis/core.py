...
class BranchModel(tf.keras.Model):
    ...
    def add_branches(self,branchNames, branchPoints=[], exact = True, target_input = False, compact = False, loop=False,num_outputs=10):
        if len(branchPoints) == 0:
            return
        else:
            newModel = branch.add(self,branchNames,branchPoints, exact=exact, target_input = target_input, compact = compact,num_outputs=num_outputs)
